...
public class UserControllerTest {
    ...
    void setUp() {
        ...
        when(userService.getAll()).thenReturn(Optional.ofNullable(users));
        when(userService.findUserByUserid(7L)).thenReturn(Optional.ofNullable(users.get(4)));
        when(userService.findUserByUserid(49L)).thenThrow(ResourceNotFoundException.class);
    }
    ...
    void whenCreatingNew_shouldReturnLink_andShouldBeStatusCreated() throws Exception {
        User new_user = new User(89L, "nouveau", "mdp");
        ArgumentCaptor<User> user_received = ArgumentCaptor.forClass(User.class);
        when(userService.saveUser(any())).thenReturn(new_user);
        ...
        verify(userService).saveUser(user_received.capture());
        assertEquals(new_user, user_received.getValue());
    }

    @Test
    void whenCreatingWithExistingId_should404() throws Exception {
        when(userService.saveUser(any())).thenThrow(ResourceAlreadyExistsException.class);
        mockMvc.perform(post("/api/users")
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void whenUpdating_shouldReceiveUserToUpdate_andReturnNoContent() throws Exception {
        ...
        User updated_user = new User(initial_user.getId(), "updated", "updated");
        ArgumentCaptor<User> user_received = ArgumentCaptor.forClass(User.class);
    ...
    void whenDeletingExisting_shouldCallServiceWithCorrectId_andSendNoContent() throws Exception {
        ...
        Mockito.verify(userService).deleteUser(id_received.capture());
